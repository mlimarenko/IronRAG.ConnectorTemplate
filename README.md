...
- YAML routing: arbitrary adapter-emitted facts → canonical
  `workspace-slug/library-slug` refs.
...
cp routing.yaml.example routing.yaml  # set visible workspace/library slugs
...
[`routing.yaml.example`](routing.yaml.example) is the complete schema reference:
it lists every key, type, default, enum value, matching rule, and failure mode.
Targets use the same canonical `<workspace-slug>/<library-slug>` ref as IronRAG
MCP. Both slugs are visible in **Admin → Libraries**. UUIDs and mutable display
names are intentionally rejected.

At startup, the framework resolves the whole routing snapshot through
IronRAG's permission-filtered catalog API. It keeps UUIDs internal for content
operations and cursor state. A missing, malformed, or unauthorized ref aborts
startup before any document mutation. Hot reload activates a changed file only
after every ref resolves; otherwise the previous working snapshot remains live.
